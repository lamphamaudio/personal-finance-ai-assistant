...
class BookmarkDB:
    ...
    def _migrate(self) -> None:
        ...
        try:
            self._conn.execute("ALTER TABLE tx_history ADD COLUMN category TEXT NOT NULL DEFAULT 'Chưa phân loại'")
            self._conn.commit()
    ...
    def _migrate_vietnamese_categories(self) -> None:
        ...
        for old, new in EN_TO_VI_CATEGORY.items():
            self._conn.execute("UPDATE tx_history SET category = ? WHERE category = ?", (new, old))
            self._conn.execute("UPDATE user_overrides SET category = ? WHERE category = ?", (new, old))
            self._conn.execute("UPDATE merchant_categories SET category = ? WHERE category = ?", (new, old))
            self._conn.execute("UPDATE budget_limits SET category = ? WHERE category = ?", (new, old))
            self._conn.execute("UPDATE category_rules SET category = ? WHERE category = ?", (new, old))
            self._conn.execute("UPDATE learning_feedback SET category = ? WHERE category = ?", (new, old))
        self._conn.commit()
