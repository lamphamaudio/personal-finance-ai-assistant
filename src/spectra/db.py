...
CREATE INDEX IF NOT EXISTS idx_tx_history_date ON tx_history (date DESC);
CREATE INDEX IF NOT EXISTS idx_tx_history_category ON tx_history (category);
CREATE INDEX IF NOT EXISTS idx_tx_history_clean_name ON tx_history (clean_name);
"""
...
class BookmarkDB:
    ...
    def _migrate(self) -> None:
        ...
        for statement in (
            "ALTER TABLE tx_history ADD COLUMN IF NOT EXISTS category text NOT NULL DEFAULT 'Chưa phân loại'",
            "ALTER TABLE tx_history ADD COLUMN IF NOT EXISTS original_description text NOT NULL DEFAULT ''",
        ):
    ...
    def _migrate_vietnamese_categories(self) -> None:
        ...
        for old, new in EN_TO_VI_CATEGORY.items():
            self._conn.execute("UPDATE tx_history SET category = %s WHERE category = %s", (new, old))
            self._conn.execute("UPDATE user_overrides SET category = %s WHERE category = %s", (new, old))
            self._conn.execute("UPDATE merchant_categories SET category = %s WHERE category = %s", (new, old))
            self._conn.execute("UPDATE budget_limits SET category = %s WHERE category = %s", (new, old))
            self._conn.execute("UPDATE category_rules SET category = %s WHERE category = %s", (new, old))
            self._conn.execute("UPDATE learning_feedback SET category = %s WHERE category = %s", (new, old))
        self._conn.commit()

    def is_seen(self, tx_id: str) -> bool:
        """Return True if this transaction ID was already processed."""
        row = self._conn.execute(
            "SELECT 1 FROM seen_transactions WHERE tx_id = %s", (tx_id,)
        ).fetchone()
    ...
    def mark_seen(self, tx_id: str, source: str = "CSV") -> None:
        ...
        self._conn.execute(
            """
            INSERT INTO seen_transactions (tx_id, source, seen_at)
            VALUES (%s, %s, %s)
    ...
    def save_merchant_category(self, clean_name: str, category: str) -> None:
        ...
        self._conn.execute(
            """
            INSERT INTO merchant_categories (clean_name, category)
            VALUES (%s, %s)
    ...
    def update_category_rule(
        ...
        *,
        ...
        cur = self._conn.execute(
            f"UPDATE category_rules SET {', '.join(updates)} WHERE id = %s",
            params,
        )
        self._conn.commit()
        if cur.rowcount <= 0:
    ...
    def delete_category_rule(self, rule_id: int) -> bool:
        ...
        cur = self._conn.execute("DELETE FROM category_rules WHERE id = %s", (int(rule_id),))
        self._conn.commit()
        if cur.rowcount > 0:
            self._normalize_rule_priorities()
    ...
    def save_overrides(self, overrides: dict[str, dict[str, str]]) -> None:
        ...
        self._conn.executemany(
            """
            INSERT INTO user_overrides (original_description, category, clean_name)
            VALUES (%s, %s, %s)
    ...
    def reapply_learning_to_history(self) -> dict[str, int]:
        ...
        for tx_id, original_description, clean_name, current_category in rows:
            ...
            self._conn.execute(
                "UPDATE tx_history SET clean_name = %s, category = %s WHERE tx_id = %s",
