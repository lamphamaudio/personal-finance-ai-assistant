...
def refresh_dashboard(sheets_client: Any) -> None:
    ...
    if recurring_items:
        subs = sorted(
            [v for v in recurring_items.values() if v["type"] == RECURRING_SUBSCRIPTION],
            key=lambda x: x["total"],
        )
        income = sorted(
            [v for v in recurring_items.values() if v["type"] == RECURRING_INCOME],
        ...
        recur_rows.append(["", "", ""])
        recur_rows.append(["Tổng đăng ký/tháng", "", sub_total])
        recur_rows.append(["Tổng thu nhập định kỳ/tháng", "", inc_total])
