...
from spectra.categories import RECURRING_INCOME, RECURRING_SUBSCRIPTION
...
_SUBSCRIPTION_MERCHANTS = {
    ...
    # Telecom & Internet
    "vodafone", "iliad", "ho mobile", "viettel", "mobifone", "vinaphone", "fpt telecom",
...
_INCOME_PATTERNS = [
    ...
    re.compile(r"\blương\b", re.IGNORECASE),
    re.compile(r"\blu[oơ]ng\b", re.IGNORECASE),
    re.compile(r"\bthanh\s+to[aá]n\s+l[uư][oơ]ng\b", re.IGNORECASE),
...
def _detect_static_pattern(
    ...
    """Return a Vietnamese recurring label based on hardcoded pattern matching."""
                    ...
                    for p_date_str, p_amount in past_entries:
                        ...
                        if abs(t.amount - p_amount) <= max(3.0, abs(p_amount * 0.15)):
                            ...
                            if (6 <= days_diff <= 8) or (27 <= days_diff <= 32) or (355 <= days_diff <= 370):
                                t.recurring = RECURRING_INCOME if t.amount > 0 else RECURRING_SUBSCRIPTION
                                matched = True
                                break
