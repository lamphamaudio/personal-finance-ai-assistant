...
def generate_html_report(transactions: list[Any], output_dir: str | Path = "data") -> Path:
    ...
    for t in transactions:
        ...
        # Tags formatting
        tag_html = ""
        if t.recurring == RECURRING_SUBSCRIPTION:
            tag_html = '<span class="tag subscription">Đăng ký định kỳ</span>'
        elif t.recurring == RECURRING_INCOME:
            tag_html = '<span class="tag salary">Thu nhập định kỳ</span>'
            
        # FX String
        fx_html = ""
        if t.original_amount is not None:
            fx_html = f'<div class="fx-note">Đã quy đổi từ {t.original_amount:.2f} {t.original_currency}</div>'
